 ...
 * 联系人
 */
public class ContactsFragment extends MSBaseFragment<FragContactsLayoutBinding> implements OnQuickSideBarTouchListener {
    ...
    protected void initView() {
        msVBinding.textView.setTextSize(22);
        Typeface face = Typeface.createFromAsset(getResources().getAssets(),
                "fonts/mw_bold.ttf");
        msVBinding.textView.setTypeface(face);
    ...
    protected void initListener() {
        ...
        initAdapter(msVBinding.recyclerView, friendAdapter);
        ...
        //成员刷新监听
        MSIM.getInstance().getChannelManager().addOnRefreshChannelInfo("contacts_fragment_refresh_channel", (channel, isEnd) -> {
            if (channel != null) {
        ...
        msVBinding.searchIv.setOnClickListener(view -> {
        ...
        //监听刷新通讯录
        EndpointManager.getInstance().setMethod("", EndpointCategory.msRefreshMailList, object -> {
            resetHeaderData();
            return null;
        });

        EndpointManager.getInstance().setMethod(MSConstants.refreshContacts, object -> {
            getContacts();
            return null;
        });
    }
    ...
    private void getContacts() {
        List<MSChannel> allList = MSIM.getInstance().getChannelManager().getWithFollowAndStatus(MSChannelType.PERSONAL, 1, 1);
        ...
        for (int i = 0, size = list.size(); i < size; i++) {
            ...
            if (list.get(i).channel.channelID.equals(MSSystemAccount.system_file_helper)) {
                if (isAdded())
                    showName = getString(R.string.ms_file_helper);
                list.get(i).channel.channelName = showName;
            }
            if (list.get(i).channel.channelID.equals(MSSystemAccount.system_team)) {
                if (isAdded())
                    showName = getString(R.string.ms_system_notice);
                list.get(i).channel.channelName = showName;
            }
            if (!TextUtils.isEmpty(showName)) {
        ...
        if (MSReader.isNotEmpty(list)) {
            for (int i = 0, size = list.size(); i < size; i++) {
                if (list.get(i).pying.startsWith(letter)) {
                    msVBinding.recyclerView.scrollToPosition(i + friendAdapter.getHeaderLayoutCount());
                    break;
                }
            }
        }
        ...
        if (isAdded()) {
            ...
            for (int i = 0, size = list.size(); i < size; i++) {
                if (!TextUtils.isEmpty(list.get(i).sid) && list.get(i).sid.equals("friend")) {
                    list.get(i).badgeNum = MSSharedPreferencesUtil.getInstance().getInt(MSConfig.getInstance().getUid() + "_new_friend_count");
                    break;
                }
            }
