 ...
 * 用户
 */
public class UserModel extends MSBaseModel {
    private UserModel() {
    }
    ...
    public void addBlackList(String uid, final ICommonListener iCommonListener) {
        request(createService(UserService.class).addBlackList(uid), new IRequestResultListener<CommonResponse>() {
            @Override
            public void onSuccess(CommonResponse result) {
                MSCommonModel.getInstance().getChannel(uid, MSChannelType.PERSONAL, null);
                iCommonListener.onResult(result.status, result.msg);
            }
    ...
    public void uploadAvatar(String filePath, final IUploadBack iUploadBack) {
        String url = MSApiConfig.baseUrl + "users/" + MSConfig.getInstance().getUid() + "/avatar?uuid=" + MSTimeUtils.getInstance().getCurrentMills();
    ...
    public void getOnlineUsers() {
        request(createService(UserService.class).onlineUsers(), new IRequestResultListener<OnlineUserAndDevice>() {
            ...
            public void onSuccess(OnlineUserAndDevice result) {
                ...
                MSSharedPreferencesUtil.getInstance().putInt(MSConfig.getInstance().getUid() + "_mute_of_app", muteOfAPP);
                ...
                if (MSReader.isNotEmpty(result.friends)) {
                    if (MSReader.isNotEmpty(tempList)) {
                        ...
                        for (int i = 0, size = result.friends.size(); i < size; i++) {
                            ...
                            if (isAdd) {
                                MSChannel channel = MSIM.getInstance().getChannelManager().getChannel(result.friends.get(i).uid, MSChannelType.PERSONAL);
                                if (channel != null) {
                ...
                MSIM.getInstance().getChannelManager().saveOrUpdateChannels(list);
            }
        ...
        request(createService(UserService.class).getUserInfo(uid, groupNo), new IRequestResultListener<>() {
            ...
            public void onSuccess(UserInfo result) {
                if (result.group_member != null) {
                    ...
                    MSIM.getInstance().getChannelMembersManager().save(member);
                }
                if (iUserInfo != null) {
    ...
    public void device(){
        String deviceId = MSConstants.getDeviceID();
        request(createService(UserService.class).device(deviceId), new IRequestResultListener<>() {
            @Override
            public void onSuccess(Device result) {
                if (result != null) {
                    MSIM.getInstance().setDeviceId(String.valueOf(result.id));
                }
            }
