 ...
 * 新朋友
 */
public class NewFriendsActivity extends MSBaseActivity<ActCommonListLayoutBinding> {
    ...
    protected void initView() {
        ...
        MSSharedPreferencesUtil.getInstance().putInt(MSConfig.getInstance().getUid() + "_new_friend_count", 0);
    }
                ...
                if (view1.getId() == R.id.agreeBtn) {
                    FriendModel.getInstance().agreeFriendApply(newFriendEntity.token, (code, msg) -> {
                        if (code == HttpResponseCode.success) {
                            ...
                            MSContactsDB.getInstance().updateFriendStatus(newFriendEntity.apply_uid, 1);
                        }
                    });
                }
            }
    ...
    protected void initData() {
        ...
        List<MSChannel> friends = MSIM.getInstance().getChannelManager().getChannels(uidList);
        if (MSReader.isNotEmpty(friends)) {
            for (MSChannel channel : friends) {
