...
class FriendAdapter :
    ...
    override fun convert(holder: BaseViewHolder, item: FriendUIEntity) {
        ...
        if (!TextUtils.isEmpty(item.channel.category)) {
            if (item.channel.category == MSSystemAccount.accountCategorySystem) {
            ...
            if (item.channel.category == MSSystemAccount.accountCategoryCustomerService) {
                linearLayout.addView(
                    Theme.getChannelCategoryTV(
                        context,
            ...
            if (item.channel.category == MSSystemAccount.accountCategoryVisitor) {
                linearLayout.addView(
                    Theme.getChannelCategoryTV(
                        context,
        ...
        if (item.channel.online == 1) {
            ...
            if (item.channel.lastOffline == 0L) {
                ...
                val lastSeenTime =
                    MSTimeUtils.getInstance().getOnlineTime(item.channel.lastOffline)
                if (TextUtils.isEmpty(lastSeenTime)) {
                    holder.setGone(R.id.offlineTv, false)
                    val time = MSTimeUtils.getInstance()
        ...
        list.add(
            PopupMenuItem(context.getString(R.string.send_msg), R.mipmap.menu_chats,
                object : PopupMenuItem.IClick {
                    override fun onClick() {
                        MSIMUtils.getInstance().startChatActivity(
                            ChatViewMenu(
