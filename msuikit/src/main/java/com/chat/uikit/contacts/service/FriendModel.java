 ...
 * 好友管理
 */
public class FriendModel extends MSBaseModel {
    ...
    public void syncFriends(final ICommonListener iCommonListener) {
        String key = String.format("%s_friend_sync_version", MSConfig.getInstance().getUid());
        long version = MSSharedPreferencesUtil.getInstance().getLong(key);
            ...
            public void onSuccess(List<UserInfo> list) {
                if (MSReader.isNotEmpty(list)) {
                    ...
                    for (int i = 0, size = list.size(); i < size; i++) {
                        ...
                        channel.channelType = MSChannelType.PERSONAL;//指定为单聊信息
                        ...
                        hashMap.put(MSChannelExtras.revokeRemind, list.get(i).revoke_remind);
                        hashMap.put(MSChannelExtras.screenshot, list.get(i).screenshot);
                        hashMap.put(MSChannelExtras.sourceDesc, list.get(i).source_desc);
                        hashMap.put(MSChannelExtras.chatPwdOn, list.get(i).chat_pwd_on);
                        hashMap.put(MSChannelExtras.vercode, list.get(i).vercode);
                    ...
                    MSSharedPreferencesUtil.getInstance().putLong(key, tempVersion);
                    //将好友信息设置到sdk
                    dispatchQueuePool.execute(() -> {
                        MSIM.getInstance().getChannelManager().saveOrUpdateChannels(channels);
                    });
    ...
    public void saveNewFriendsMsg(String contentJson) {
        ...
        if (newFriendEntity1 != null && !TextUtils.isEmpty(newFriendEntity1.apply_uid)) {
            ...
            newFriendEntity1.created_at = MSTimeUtils.getInstance().getNowDate1();
            ApplyDB.getInstance().update(newFriendEntity1);
        } else {
            newFriendEntity.created_at = MSTimeUtils.getInstance().getNowDate1();
            ApplyDB.getInstance().insert(newFriendEntity);
        }
        int new_friend_count = MSSharedPreferencesUtil.getInstance().getInt(MSConfig.getInstance().getUid() + "_new_friend_count");
        new_friend_count++;
        MSSharedPreferencesUtil.getInstance().putInt(MSConfig.getInstance().getUid() + "_new_friend_count", new_friend_count);
        EndpointManager.getInstance().invokes(EndpointCategory.msRefreshMailList, null);
    }
