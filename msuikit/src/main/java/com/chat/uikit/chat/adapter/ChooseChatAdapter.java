...
public class ChooseChatAdapter extends BaseQuickAdapter<ChooseChatActivity.ChooseChatEntity, BaseViewHolder> {
    ...
    protected void convert(@NonNull BaseViewHolder helper, ChooseChatActivity.ChooseChatEntity item) {
        ...
        if (item.uiConveursationMsg.getMsChannel() != null) {
            String showName = item.uiConveursationMsg.getMsChannel().channelRemark;
            if (TextUtils.isEmpty(showName))
                showName = item.uiConveursationMsg.getMsChannel().channelName;
            helper.setText(R.id.nameTv, showName);
            avatarView.showAvatar(item.uiConveursationMsg.getMsChannel());
            ...
            MSIM.getInstance().getChannelManager().fetchChannelInfo(item.uiConveursationMsg.channelID, item.uiConveursationMsg.channelType);
