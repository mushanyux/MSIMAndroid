...
import com.chat.uikit.chat.adapter.MSChatToolBarAdapter
...
import com.chat.uikit.robot.entity.MSRobotGIFEntity
...
class ChatPanelManager(
    ...
    private val loginUID = MSConfig.getInstance().uid
    ...
    // 工具栏
    private var toolBarAdapter: MSChatToolBarAdapter? = null
...
//        chatView.visibility=View.VISIBLE
    ...
    fun showReplyLayout(mMsg: MSMsg) {
        var showName: String? = ""
        if (mMsg.from != null) {
    ...
    fun showEditLayout(mMsg: MSMsg) {
        val textModel = mMsg.baseContentMsgModel as MSTextContent
    ...
    fun initRefreshListener() {
        MSIM.getInstance().channelMembersManager.addOnAddChannelMemberListener(this.eventKey) { list ->
        ...
        MSIM.getInstance().channelMembersManager.addOnRefreshChannelMemberInfo(
            this.eventKey
        ) { mChannelMember, _ ->
            if (mChannelMember != null
                ...
                && iConversationContext.chatChannelInfo.channelType == MSChannelType.GROUP
            ) {
                //禁言
        ...
        MSIM.getInstance().channelManager.addOnRefreshChannelInfo(
            this.eventKey
        ) { mChannel, _ ->
            if (mChannel.channelType == iConversationContext.chatChannelInfo.channelType && mChannel.channelID.equals(
                ...
                showOrHideForbiddenView()
                // 封禁群
                if (mChannel.status == MSChannelStatus.statusDisabled) {
                    showBan()
                } else {
                    hideBan()
                }
    ...
    private fun showForbiddenTimer(totalTime: Long) {
        ...
        val timerTask: TimerTask = object : TimerTask() {
            override fun run() {
                val nowTime = MSTimeUtils.getInstance().currentSeconds
    ...
    fun showOrHideForbiddenView() {
        ...
        if (iConversationContext.chatChannelInfo.channelType == MSChannelType.CUSTOMER_SERVICE) {
            hideBan()
            return
        }
        ...
        val mChannelMember = MSIM.getInstance().channelMembersManager.getMember(
            iConversationContext.chatChannelInfo.channelID,
            iConversationContext.chatChannelInfo.channelType,
            this.loginUID
        )
        if (mChannelMember != null) {
            if (mChannelMember.role == MSChannelMemberRole.admin) {
                hideForbiddenView()
            } else {
                if (mChannel != null && mChannel.forbidden == 1) {
                    if (mChannelMember.role == MSChannelMemberRole.manager) {
    ...
    fun chatAvatarClick(uid: String, isLongClick: Boolean) {
        if (isLongClick) {
            ...
            if (iConversationContext.chatChannelInfo.channelType == MSChannelType.GROUP) {
                val loginMember = MSIM.getInstance().channelMembersManager.getMember(
                    iConversationContext.chatChannelInfo.channelID,
                    iConversationContext.chatChannelInfo.channelType,
                    this.loginUID
                )
                if (loginMember != null) {
                    if ((iConversationContext.chatChannelInfo.forbidden == 1 && loginMember.role == MSChannelMemberRole.normal) || loginMember.forbiddenExpirationTime > 0) {
                        return
                    }
                }
                val member =
                    MSIM.getInstance().channelMembersManager.getMember(
                        iConversationContext.chatChannelInfo.channelID,
                        iConversationContext.chatChannelInfo.channelType,
                        uid
                    )
    ...
    fun onDestroy() {
        ...
        MSIM.getInstance().robotManager.removeRefreshRobotMenu(iConversationContext.chatChannelInfo.channelID)
        MSIM.getInstance().channelManager.removeRefreshChannelInfo(this.eventKey)
        MSIM.getInstance().channelMembersManager.removeRefreshChannelMemberInfo(this.eventKey)
        MSIM.getInstance().channelMembersManager.removeAddChannelMemberListener(this.eventKey)
    }
    ...
    private fun setProgress(progress: Float) {
        ...
        if (iConversationContext.chatChannelInfo.channelType == MSChannelType.PERSONAL) {
    ...
    private fun initTool() {
        toolBarAdapter = MSChatToolBarAdapter()
        ...
        val toolBarList = EndpointManager.getInstance()
            .invokes<ChatToolBarMenu>(EndpointCategory.msChatToolBar, iConversationContext)
        ...
        toolBarAdapter?.setOnItemChildClickListener { adapter1: BaseQuickAdapter<*, *>, view: View, position: Int ->
            if (view.id == R.id.imageView) {
                SingleClickUtil.determineTriggerSingleClick(view, 500) {
                    ...
                    // 如果点击的是@
                    if (mChatToolBarMenu.sid == "ms_chat_toolbar_remind") {
                    ...
                    //如果点击的是更多
                    if (mChatToolBarMenu.sid == "ms_chat_toolbar_more") {
                        val path = ImageUtils.getInstance().newestPhoto
                        val oldPath =
                            MSSharedPreferencesUtil.getInstance().getSP("new_img_path")
                    ...
                    if (mChatToolBarMenu.sid == "ms_chat_toolbar_voice") {
                        checkPermission(
                            iConversationContext.chatActivity,
                            mChatToolBarMenu,
                            position,
    ...
    private fun initRobotMenu() {
        ...
        val menus = MSRobotModel.getInstance().getRobotMenus(
            iConversationContext.chatChannelInfo.channelID,
            iConversationContext.chatChannelInfo.channelType
        )
        ...
        menuLayout.setOnClickListener {
            ...
            if (robotMenuAdapter!!.data.isEmpty()) {
                val tempMenu: List<MSRobotMenuEntity> =
                    MSRobotModel.getInstance().getRobotMenus(
                        iConversationContext.chatChannelInfo.channelID,
                        iConversationContext.chatChannelInfo.channelType
                    )
        ...
        robotMenuAdapter!!.setOnItemClickListener { _: BaseQuickAdapter<*, *>?, _: View?, position: Int ->
            ...
            if (menu != null) {
                ...
                val textContent = MSTextContent(menu.cmd)
                val list: MutableList<MSMsgEntity> =
                    ArrayList()
                val entity = MSMsgEntity()
                ...
                val msMsg = MSMsg()
                msMsg.channelID = iConversationContext.chatChannelInfo.channelID
                msMsg.channelType = iConversationContext.chatChannelInfo.channelType
                msMsg.type = textContent.type
                msMsg.baseContentMsgModel = textContent
                msMsg.channelInfo = iConversationContext.chatChannelInfo
                msMsg.robotID = menu.robot_id
                Log.e("ID是：", menu.robot_id)
                MSSendMsgUtils.getInstance().sendMessage(msMsg)
            }
        }
        // 监听机器人刷新菜单
        MSIM.getInstance().robotManager.addOnRefreshRobotMenu(iConversationContext.chatChannelInfo.channelID) {
    ...
    private fun checkRobotMenu(iConversationContext: IConversationContext) {
        val robotMembers =
            MSIM.getInstance().channelMembersManager.getRobotMembers(
                iConversationContext.chatChannelInfo.channelID,
                iConversationContext.chatChannelInfo.channelType
            )
...
//            }
    ...
    private fun resetMenuHeader() {
        parentView.post {
            ...
            //  menuHeaderView!!.layoutParams.height -= MSConstants.getKeyboardHeight()
    ...
    private fun initRemind() {
        ...
        if (iConversationContext.chatChannelInfo.channelType == MSChannelType.PERSONAL) return
...
//                remindHeaderView!!.layoutParams.height -= MSConstant.getKeyboardHeight()
//            }
    ...
    private fun initRobotGIF() {
        ...
        robotGIFAdapter!!.setOnItemClickListener { adapter, _, position ->
            val entity = adapter.data[position] as MSRobotGIFEntity
            if (entity.isNull) return@setOnItemClickListener
            hideRobotView()
            val stickerContent = MSGifContent()
    ...
    private fun initListener() {
        ...
        sendIV.setOnClickListener {
            ...
            if (!TextUtils.isEmpty(content)) {
                ...
                val textMsgModel = MSTextContent(content)

                val list = editText.allUIDs
                if (list != null && list.isNotEmpty()) {
                    val mMentionInfo = MSMentionInfo()
                ...
                if (nowTime - lastInputTime >= 5 && !TextUtils.isEmpty(s)) {
                    ...
                    if (iConversationContext.chatChannelInfo.channelType == MSChannelType.GROUP) {
                        ...
                        if (channel?.localExtra != null) {
                            var beDeleted = 0
                            var beBlacklist = 0
                            if (channel.localExtra.containsKey(MSChannelExtras.beBlacklist)) {
                                beBlacklist =
                                    channel.localExtra[MSChannelExtras.beBlacklist] as Int
                            }
                            if (channel.localExtra.containsKey(MSChannelExtras.beDeleted)) {
                                beDeleted =
                                    channel.localExtra[MSChannelExtras.beDeleted] as Int
                            }
                    ...
                    lastInputTime = MSTimeUtils.getInstance().currentSeconds
                }
            }
        })
    }
...
//                }
                val mRobot =
                    MSIM.getInstance().robotManager.getWithUsername(username.lowercase(Locale.getDefault()))
...
//                        .showOrHide(closeSearchLottieIV, false, true)
                    ...
                    val list: MutableList<MSRobotEntity> = ArrayList()
                    list.add(
                        MSRobotEntity(
                            "",
                            username,
                    ...
                    MSRobotModel.getInstance().syncRobot(2, list)
                    hideRobotView()
                }
            } else {
                CommonAnim.getInstance().showOrHide(hitTv, false, true)
//                CommonAnim.getInstance().showOrHide(closeSearchLottieIV, false, true)
        ...
        if (iConversationContext.chatChannelInfo.channelType == MSChannelType.GROUP && isSearchGroupMembers) {
...
//        })
    ...
    private fun searchRobotGif(searchKey: String, username: String) {
        ...
        MSRobotModel.getInstance().inlineQuery(
            inlineQueryOffset,
            username,
            searchKey,
            iConversationContext.chatChannelInfo.channelID,
            iConversationContext.chatChannelInfo.channelType
        ) { _: Int, _: String?, result: MSRobotInlineQueryResult? ->
    ...
    fun hideRemindView() {
        if (iConversationContext.chatChannelInfo.channelType == MSChannelType.GROUP && remindRecycleView!!.visibility != View.GONE) {
...
//            initRobotGIF(iConversationContext!!)
    ...
    private fun getEmojiLayout(): View {
        ...
        var height = MSConstants.getKeyboardHeight()
        if (height == 0) {
    ...
    private fun checkPermission(
        ...
        position: Int,
        adapter1: MSChatToolBarAdapter
    ) {
        val desc = String.format(
            activity.getString(R.string.microphone_permissions_des),
            activity.getString(R.string.app_name)
        )
        MSPermissions.getInstance().checkPermissions(object : MSPermissions.IPermissionResult {
    ...
    private fun showNewImgDialog(path: String) {
        MSSharedPreferencesUtil.getInstance().putSP("new_img_path", path)
    ...
    private fun initMultipleChoiceView() {
        ...
        forwardView.setOnClickListener {
                ...
                BottomSheetItem(
                    ...
                    object : BottomSheetItem.IBottomSheetClick {
                        override fun onClick() {
                            ...
                            //合并转发
                            val forwardContent =
                                MSMultiForwardContent()
                            forwardContent.channelType =
                                iConversationContext.chatChannelInfo.channelType
                            val list: MutableList<MSMsg> =
                                ArrayList()
                            ...
                            while (i < itemCount) {
                                if (chatAdapter.getItem(i).isChecked) {
                                    list.add(chatAdapter.getItem(i).msMsg)
                                    if (iConversationContext.chatChannelInfo.channelType == MSChannelType.PERSONAL) {
                                        var isAdd: Boolean
                                        ...
                                        if (isAdd) {
                                            if (chatAdapter.getItem(i).msMsg.from == null) {
                                                val mChannel = MSChannel()
                                                mChannel.channelID =
                                                    chatAdapter.getItem(i).msMsg.fromUID
                                                chatAdapter.getItem(i).msMsg.from = mChannel
                                            }
                                            forwardContent.userList.add(chatAdapter.getItem(i).msMsg.from)
                                        }
                                    }
                                }
                                i++
                                    ...
                                    ChooseChatMenu(
                                        ChatChooseContacts { channelList: List<MSChannel>? ->
                                            if (!channelList.isNullOrEmpty()) {
                                                ...
                                                for (mChannel: MSChannel in channelList) {
                ...
                BottomSheetItem(
                    ...
                    object : BottomSheetItem.IBottomSheetClick {
                        override fun onClick() {
                            ...
                            //逐条转发
                            val list: MutableList<MSMessageContent> =
                                ArrayList()
                            var i = 0
                            val itemCount: Int = chatAdapter.itemCount
                            while (i < itemCount) {
                                if (chatAdapter.getItem(i).isChecked) {
                                    if ((chatAdapter.getItem(i).msMsg.type == MSContentType.MS_TEXT
                                                ) || (chatAdapter.getItem(i).msMsg.type == MSContentType.MS_IMAGE
                                                ) || (chatAdapter.getItem(i).msMsg.type == MSContentType.MS_GIF)
                                    ) list.add(chatAdapter.getItem(i).msMsg.baseContentMsgModel) else {
                                        val textContent =
                                            MSTextContent(chatAdapter.getItem(i).msMsg.baseContentMsgModel.displayContent)
            ...
            MSDialogUtils.getInstance().showBottomSheet(
                iConversationContext.chatActivity,
        ...
        deleteView.setOnClickListener {
            val chatAdapter = iConversationContext.chatAdapter
            val list: MutableList<MSMsg> = ArrayList()
            ...
            run {
                ...
                while (i < itemCount) {
                    if (chatAdapter.getItem(i).isChecked) {
                        list.add(chatAdapter.getItem(i).msMsg)
                        ids.add(chatAdapter.getItem(i).msMsg.clientMsgNO)
                    }
                    i++
                }
            }
            if (list.isNotEmpty()) {
                MSDialogUtils.getInstance().showDialog(
                    ...
                    if (index == 1) {
                        MSIM.getInstance().msgManager.deleteWithClientMsgNos(ids)
