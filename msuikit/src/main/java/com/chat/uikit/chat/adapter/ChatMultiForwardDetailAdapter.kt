...
class ChatMultiForwardDetailAdapter(
    ...
    override fun convert(holder: BaseViewHolder, item: ChatMultiForwardEntity) {
        when (item.itemType) {
            ...
            0 -> {
                ...
                val isGone =
                    (holder.bindingAdapterPosition != 0 && data[holder.bindingAdapterPosition - 1].itemType == 0
                            && item.msg.baseContentMsgModel != null
                            && data[holder.bindingAdapterPosition - 1].msg.baseContentMsgModel != null
                            && !TextUtils.isEmpty(item.msg.fromUID)
                ...
                when (item.msg.baseContentMsgModel.type) {
                    MSContentType.MS_IMAGE -> {
                        ...
                        val imgMsgModel = item.msg.baseContentMsgModel as MSImageContent
                        var showUrl: String
                    ...
                    MSContentType.MS_VIDEO -> {
                        ...
                        val videoModel = item.msg.baseContentMsgModel as MSVideoContent
                        var coverURL = ""
                        if (!TextUtils.isEmpty(videoModel.coverLocalPath)) {
                            ...
                            coverURL = MSApiConfig.getShowUrl(videoModel.cover)
                        }
                        GlideUtils.getInstance().showImg(
                            context,
                            coverURL,
                            ...
                            .setOnClickListener {
                                val videoUrl: String =
                                    if (!TextUtils.isEmpty(videoModel.localPath)) {
                                        ...
                                        if (!file.exists()) {
                                            MSApiConfig.getShowUrl(videoModel.url)
                                        } else videoModel.localPath
                                    } else MSApiConfig.getShowUrl(videoModel.url)
                    ...
                    MSContentType.MS_GIF -> {
                        ...
                        val msGifContent =
                            item.msg.baseContentMsgModel as MSGifContent
                        GlideUtils.getInstance().showImg(
                            context,
                            MSApiConfig.getShowUrl(msGifContent.url),
    ...
    private fun showImages(
        uri: String,
        imageView: ImageView,
        messageContent: MSMessageContent
    ) {
        //查看大图
        ...
        MSDialogUtils.getInstance().showImagePopup(
            context,
            tempImgList,
            imgList,
            imageView,
