...
public class AllMembersAdapter extends BaseQuickAdapter<AllGroupMemberEntity, BaseViewHolder> {
    ...
    protected void convert(@NotNull BaseViewHolder baseViewHolder, AllGroupMemberEntity entity) {
        MSChannelMember channelMember = entity.getChannelMember();
        ...
        avatarView.showAvatar(channelMember.memberUID, MSChannelType.PERSONAL, channelMember.memberAvatar);
        if (channelMember.role == MSChannelMemberRole.admin) {
        ...
        } else if (channelMember.role == MSChannelMemberRole.manager) {
            roleTv.setVisibility(View.VISIBLE);
