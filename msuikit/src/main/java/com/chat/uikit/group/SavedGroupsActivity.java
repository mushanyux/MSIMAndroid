 ...
 * 我保存的群组
 */
public class SavedGroupsActivity extends MSBaseActivity<ActCommonListLayoutBinding> implements GroupContract.GroupView {
    ...
    protected void initView() {
        ...
        initAdapter(msVBinding.recyclerView, groupAdapter);
    }
    ...
    protected void initListener() {
        ...
        groupAdapter.setOnItemChildClickListener((adapter, view1, position) -> {
            ...
            if (channel != null) {
                MSIMUtils.getInstance().startChatActivity(new ChatViewMenu(this, channel.group_no, MSChannelType.GROUP, 0, true));
            }
        });
    }
    ...
    public void setMyGroups(List<GroupEntity> list) {
        if (MSReader.isEmpty(list)) {
            ...
            List<MSChannel> channels = MSIM.getInstance().getChannelManager().getWithChannelIdsAndChannelType(channelIds, MSChannelType.GROUP);
            for (int i = 0, size = list.size(); i < size; i++) {
