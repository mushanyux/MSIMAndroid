 ...
 * 群成员适配器
 */
public class GroupMemberAdapter extends BaseQuickAdapter<MSChannelMember, BaseViewHolder> {
    public GroupMemberAdapter(@Nullable List<MSChannelMember> data) {
        super(R.layout.item_group_member_layout, data);
    }

    @Override
    protected void convert(@NonNull BaseViewHolder helper, MSChannelMember item) {
        if (item.memberUID.equalsIgnoreCase("-1")) {
            ...
            avatarView.showAvatar(item.memberUID, MSChannelType.PERSONAL, item.memberAvatarCacheKey);
            helper.setGone(R.id.handlerIv, true);
            helper.setGone(R.id.userLayout, false);
            if (item.role == MSChannelMemberRole.admin) {
            ...
            } else if (item.role == MSChannelMemberRole.manager) {
