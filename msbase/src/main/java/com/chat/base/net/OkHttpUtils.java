...
import com.chat.base.utils.MSNetUtil;
...
public class OkHttpUtils {
    ...
    public OkHttpClient getOkHttpClient() {
        if (sOkHttpClient == null) {
            synchronized (OkHttpUtils.class) {
                Cache cache = new Cache(new File(MSBaseApplication.getInstance().getContext().getCacheDir(), "HttpCache"),
                        1024 * 1024 * 100);
                if (sOkHttpClient == null) {
    ...
    private final Interceptor mRewriteCacheControlInterceptor = chain -> {
        ...
        if (!MSNetUtil.isNetworkAvailable(MSBaseApplication.getInstance().getContext())) {
