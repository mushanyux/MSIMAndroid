...
public class CustomImageViewerPopup extends ImageViewerPopupView {
    ...
    private final MSMsg msg;
    private final int flame;

    public CustomImageViewerPopup(@NonNull Context context, int flame, MSMsg msg, List<ImagePopupBottomSheetItem> list, IImgPopupMenu iImgPopupMenu) {
        super(context);
    ...
    protected void onCreate() {
        ...
        MSIM.getInstance().getMsgManager().addOnDeleteMsgListener("view_img", deletedMsg -> {
...
//    }
    ...
    public void showLongClickDialog(int position, Object url) {
        ...
        if (list != null) {
            ...
            MSDialogUtils.getInstance().showBottomSheet(getContext(), context.getString(R.string.str_choose), false, sheetItemList);
        } else {

            MSDialogUtils.getInstance().showChatImageBottomViewDialog(context, String.valueOf(url), new MSDialogUtils.IImageBottomClick() {
                @Override
            ...
            Glide.with(context)
                    ...
                    .into(new CustomTarget<Bitmap>(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL) {
                        ...
                        public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
                            ...
                            MSToastUtils.getInstance().showToastNormal(context.getString(R.string.saved_album));
                        }
    ...
    public void dismiss() {
        super.dismiss();
        MSIM.getInstance().getMsgManager().removeDeleteMsgListener("view_img");
        if (msg != null && msg.flame == 1) {
            boolean disable_screenshot;
            String uid = MSConfig.getInstance().getUid();
            if (!TextUtils.isEmpty(uid)) {
                disable_screenshot = MSSharedPreferencesUtil.getInstance().getBoolean(uid + "_disable_screenshot");
            } else {
                disable_screenshot = MSSharedPreferencesUtil.getInstance().getBoolean("disable_screenshot");
            }
