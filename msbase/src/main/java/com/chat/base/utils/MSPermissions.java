...
public class MSPermissions {
    private MSPermissions() {
    }

    private static class PermissionsBinder {
        final static MSPermissions permissions = new MSPermissions();
    }

    public static MSPermissions getInstance() {
    ...
    public void checkPermissions(final IPermissionResult iPermissionResult, FragmentActivity activity, String authDesc, String... permissions) {
        ...
        rxPermissions.request(permissions).subscribe(aBoolean -> {
            if (!aBoolean) {
                MSDialogUtils.getInstance().showDialog(activity, activity.getString(R.string.authorization_request), authDesc ,false,activity.getString(R.string.cancel), activity.getString(R.string.to_set),0, Theme.colorAccount, index -> {
                    if (index == 1) {
