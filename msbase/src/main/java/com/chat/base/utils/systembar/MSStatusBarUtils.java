...
public class MSStatusBarUtils {

    public static boolean supportTransparentStatusBar() {
        return MSOSUtils.isMiui()
                || MSOSUtils.isFlyme()
                || (MSOSUtils.isOppo() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)
    ...
    public static void transparentStatusBar(Window window) {
        if (MSOSUtils.isMiui() || MSOSUtils.isFlyme()) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                transparentStatusBarAbove21(window);
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            }
        } else if ((MSOSUtils.isOppo() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)) {
            transparentStatusBarAbove21(window);
    ...
    public static void setLightMode(Window window) {
        ...
        } else if (MSOSUtils.isFlyme()) {
            setFlymeStatusBarDarkMode(window, false);
        } else if (MSOSUtils.isOppo()) {
