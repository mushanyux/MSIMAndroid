...
public class MSDialogUtils {
    private MSDialogUtils() {
    }

    private static class DialogUtilsBinder {
        static MSDialogUtils utils = new MSDialogUtils();
    }

    public static MSDialogUtils getInstance() {
...
//            window.decorView.setBackgroundColor(Color.RED)
        ...
        sureBtn.setOnClickListener(view1 -> {
            DownloadApkUtils.getInstance().downloadAPK(MSBaseApplication.getInstance().getContext(),
                    versionEntity.app_version,
                    MSApiConfig.getShowUrl(versionEntity.download_url));
            alertDialog.dismiss();
...
//            scrimPopupWindow.dimBehind();
    ...
    public BasePopupView showImagePopup(Context context, MSMsg msg, List<Object> tempImgList, List<ImageView> imgList, ImageView imageView, int index, List<ImagePopupBottomSheetItem> list, final CustomImageViewerPopup.IImgPopupMenu iImgPopupMenu, final IImagePopupListener iImagePopupListener) {
