...
//        imageView.setStrokeWidth(AndroidUtilities.dp(1));
    ...
    public void showAvatar(MSChannel channel) {
        showAvatar(channel, false);
    }

    public void showAvatar(MSChannel channel, boolean showOnlineStatus) {
        String avatarCacheKey = channel.avatarCacheKey;
        String url;
        if (!TextUtils.isEmpty(channel.avatar) && channel.avatar.contains("/")) {
            url = MSApiConfig.getShowUrl(channel.avatar);
        } else {
    ...
    private String getAvatarURL(String channelID, byte channelType) {
        String filePath = MSConstants.avatarCacheDir + channelType + "_" + channelID;
        File file = new File(filePath);
        if (file.exists()) {
            return filePath;
        } else {
            String url = MSApiConfig.getShowAvatar(channelID, channelType);
            return url;
        }
    }
