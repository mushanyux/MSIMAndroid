 ...
 * 消息列表item
 */
public class MSUIChatMsgItemEntity {
    public MSMsg msMsg; // 本条消息对象
    ...
    public MSMsg previousMsg; // 上一条消息
    public MSMsg nextMsg; // 下一条消息
    ...
    public MSUIChatMsgItemEntity(IConversationContext conversationContext, MSMsg msMsg, ILinkClick iLinkClick) {
        this.msMsg = msMsg;
        this.iLinkClick = iLinkClick;
        if (msMsg != null) {
            try {
                formatSpans(conversationContext, msMsg);
    ...
    private String getContent() {
        String showContent = msMsg.baseContentMsgModel.getDisplayContent();
        if (msMsg.remoteExtra.contentEditMsgModel != null && !TextUtils.isEmpty(msMsg.remoteExtra.contentEditMsgModel.getDisplayContent())) {
            showContent = msMsg.remoteExtra.contentEditMsgModel.getDisplayContent();
        }
        return showContent;
    }

    public void formatSpans(IConversationContext conversationContext, MSMsg msMsg) {
        if (msMsg.type != MSContentType.MS_TEXT
                || msMsg.baseContentMsgModel == null) {
            return;
        }
        ...
        if (MSReader.isNotEmpty(msMsg.baseContentMsgModel.entities)) {
            for (MSMsgEntity entity : msMsg.baseContentMsgModel.entities) {
                    ...
                    NormalClickableSpan clickableSpan = new NormalClickableSpan(true, ContextCompat.getColor(context, R.color.blue), new NormalClickableContent(types, content), view -> {
                        ...
                        if (StringUtils.isMobile(content)) {
                            ...
                            list.add(new
                                        ...
                                        MSToastUtils.getInstance().showToastNormal(context.getString(R.string.copyed));
                                    })
                            );
                            ...
                            MSDialogUtils.getInstance().showBottomSheet(context, displaySpans, false, list);
                            return;
                        }
                        ...
                        Intent intent = new Intent(conversationContext.getChatActivity(), MSWebViewActivity.class);
            ...
            if (msMsg.baseContentMsgModel.mentionInfo != null) {
                for (String uid : msMsg.baseContentMsgModel.mentionInfo.uids) {

                    String showName = "";
                    MSChannelMember member = MSIM.getInstance().getChannelMembersManager().getMember(conversationContext.getChatChannelInfo().channelID, conversationContext.getChatChannelInfo().channelType, uid);
                    if (member != null) {
                    ...
                    if (!TextUtils.isEmpty(showName)) {
                        ...
                        if (index >= 0) {
                            String groupNo = "";
                            if (msMsg.channelType == MSChannelType.GROUP) {
                                groupNo = msMsg.channelID;
                            }
                            showName = showName + " ";
            ...
            if (msMsg.baseContentMsgModel.mentionAll == 1) {
