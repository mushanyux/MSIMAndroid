...
public abstract class MSBaseActivity<MSVBinding extends ViewBinding> extends SwipeBackActivity {
    private RadialProgressView titleRightLoadingIv;
    private View titleRightLayout;
    private Button rightBtn;
    protected LoadingPopupView loadingPopup;
    protected View view;
    protected MSVBinding msVBinding;
...
//        }
        boolean na = MSStatusBarUtils.isNavigationBarExist(this);
        MSMultiLanguageUtil.getInstance().setConfiguration();
//        adaptTheme(false);
        msVBinding = getViewBinding();
        setContentView(msVBinding.getRoot());
        initSwipeBackFinish();
        initPresenter();
...
//        setStatusBarColor();
    ...
    protected abstract MSVBinding getViewBinding();
    ...
    //显示一个弹框
    protected void showDialog(String content, MSDialogUtils.IClickListener iClickListener) {
        MSDialogUtils.getInstance().showDialog(this, "", content, true, "", "", 0, 0, iClickListener);
    }

    protected void showSingleBtnDialog(String content) {
        MSDialogUtils.getInstance().showSingleBtnDialog(this, "", content, "", null);
    }

    protected void showToast(String content) {
        MSToastUtils.getInstance().showToastFail(content);
    }
...
//        titleRightIvLeft.setOnClickListener(v -> rightLeftLayoutClick());
    ...
    protected void attachBaseContext(Context newBase) {
        super.attachBaseContext(MSMultiLanguageUtil.getInstance().attachBaseContext(newBase));
    }
}
