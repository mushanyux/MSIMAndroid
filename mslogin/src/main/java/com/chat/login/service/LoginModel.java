 ...
 * 登录model
 */
public class LoginModel extends MSBaseModel {
    private LoginModel() {
    }
    ...
    void loginApp(String name, String pwd, final ILoginListener iLoginListener) {
        ...
        deviceJson.put("device_id", MSConstants.getDeviceID());
        deviceJson.put("device_name", MSDeviceUtils.getInstance().getDeviceName());
        deviceJson.put("device_model", MSDeviceUtils.getInstance().getSystemModel());
    ...
    private void saveLoginInfo(UserInfoEntity userInfo) {
        MSConfig.getInstance().saveUserInfo(userInfo);
        MSConfig.getInstance().setToken(userInfo.token);
        if (!TextUtils.isEmpty(userInfo.im_token)) {
            MSConfig.getInstance().setImToken(userInfo.im_token);
        } else MSConfig.getInstance().setImToken(userInfo.token);
        MSConfig.getInstance().setUid(userInfo.uid);
        MSConfig.getInstance().setUserName(userInfo.name);
    }
}
