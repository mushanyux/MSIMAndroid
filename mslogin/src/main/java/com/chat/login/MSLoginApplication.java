...
import com.chat.login.ui.MSResetLoginPwdActivity;
import com.chat.login.ui.MSWebLoginActivity;
import com.chat.login.ui.MSWebLoginConfirmActivity;
...
public class MSLoginApplication {
    private MSLoginApplication() {

    }

    private static class LoginApplicationBinder {
        static final MSLoginApplication LOGIN = new MSLoginApplication();
    }

    public static MSLoginApplication getInstance() {
    ...
    public void init(Context context) {
            ...
            if (type.equals("loginConfirm")) {
                ...
                if (dataJson != null && dataJson.has("auth_code")) {
                    ...
                    if (!TextUtils.isEmpty(authCode)) {
                        Intent intent = new Intent(context, MSWebLoginConfirmActivity.class);
        ...
        EndpointManager.getInstance().setMethod("chow_reset_login_pwd_view", object -> {
            Intent intent = new Intent(context, MSResetLoginPwdActivity.class);
            intent.addFlags(FLAG_ACTIVITY_NEW_TASK);
            context.startActivity(intent);
            return null;
        });
        ...
        EndpointManager.getInstance().setMethod("show_web_login_desc", object -> {
            ...
            Intent intent = new Intent(activity, MSWebLoginActivity.class);
            intent.addFlags(FLAG_ACTIVITY_NEW_TASK);
            activity.startActivity(intent);
            return null;
        });
