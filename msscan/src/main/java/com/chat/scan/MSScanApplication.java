...
public class MSScanApplication {
    private MSScanApplication() {

    }

    private static class ScanApplicationBinder {
        static final MSScanApplication application = new MSScanApplication();
    }

    public static MSScanApplication getInstance() {
    ...
    public void init(Context context) {
        ...
        EndpointManager.getInstance().setMethod("ms_scan_show", object -> {
            Intent intent = new Intent(context, MSScanActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            context.startActivity(intent);
            return null;
        });
        //添加tab页扫一扫功能
        EndpointManager.getInstance().setMethod(EndpointCategory.tabMenus + "_scan", EndpointCategory.tabMenus, 99, object -> new PopupMenuItem( context.getString(R.string.ms_scan_module_scan), R.mipmap.menu_scan,() -> {
            Intent intent = new Intent(context, MSScanActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            context.startActivity(intent);
        }));
            ...
            if (menu.isJump && !TextUtils.isEmpty(result)) {
                ScanUtils.getInstance().handleScanResult(menu.activity, result, new ScanUtils.IHandleScanResult() {
                    ...
                    public void showOtherContent(String content) {
                        Intent intent = new Intent(menu.activity, MSScanOtherResultActivity.class);
                    ...
                    public void showWebView(String url) {
                        Intent intent = new Intent(menu.activity, MSWebViewActivity.class);
